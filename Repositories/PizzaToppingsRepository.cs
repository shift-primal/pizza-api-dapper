...
public class PizzaToppingsRepository(string connectionString)
{
    ...
    public async Task Add(int pizzaId, int toppingId)
    {
        using var conn = new SqliteConnection(_connectionString);
    ...
    public async Task Remove(int pizzaId, int toppingId)
    {
        using var conn = new SqliteConnection(_connectionString);
        await conn.ExecuteAsync(
